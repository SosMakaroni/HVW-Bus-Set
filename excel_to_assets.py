...
def generate_nml(row, cm, maps):
    ...
    pspr1 = [f"spriteset(ss_{ItemID}_{Color}_purchase, \"src/{PFolder}/{ItemID}_{Color}_8bpp.png\")\t\t{{tmpl{Pos1}_purchase()}}"]
    spr1 = [f"spriteset(ss_{ItemID}_{Color}, \"src/{PFolder}/{ItemID}_{Color}_8bpp.png\")\t\t{{tmpl{Pos1}_1()}}",
    ...
    spr2, spr3 = [], []
    cs_graph1 = [f"\tss_{ItemID}_{Color};"] # grafika switch

    if float(Hossz2 or 0) > 0:
        cs_graph1 = ["\tss_toldat;"] # grafika switch
        cs_graph2 = [f"\t1: ss_{ItemID}_{Color};"] # grafika switch
        if float(Hossz3 or 0) > 0:
            pspr1 = [f"spriteset(ss_{ItemID}_{Color}_purchase, \"src/{PFolder}/{ItemID}_{Color}_8bpp.png\")\t\t{{tmpl25_purchase()}}"]
            spr1 = [f"spriteset(ss_{ItemID}_{Color}_a, \"src/{PFolder}/{ItemID}_{Color}_a_8bpp.png\")\t\t{{tmpl{Pos1}_1()}}",
            ...
            spr2 = [f"spriteset(ss_{ItemID}_{Color}_b, \"src/{PFolder}/{ItemID}_{Color}_b_8bpp.png\")\t\t{{tmpl{Pos2}_1()}}",
            ...
            cs_graph2 = [f"\t1: ss_{ItemID}_{Color}_a;"] # grafika switch
            cs_graph3 = [f"\t2: ss_{ItemID}_{Color}_b;"] # grafika switch
            if float(Hossz4 or 0) > 0:
                cs_graph3 = ["\t2: ss_toldat;"] # grafika switch
                cs_graph4 = [f"\t3: ss_{ItemID}_{Color}_b;"] # grafika switch
                if float(Hossz5 or 0) > 0:
                    spr3 = [f"spriteset(ss_{ItemID}_{Color}_c, \"src/{PFolder}/{ItemID}_{Color}_c_8bpp.png\")\t\t{{tmpl{Pos3}_1()}}",
                    ...
                    cs_graph5 = [f"\t4: ss_{ItemID}_{Color}_c;"] # grafika switch
                    if float(Hossz6 or 0) > 0:
                        cs_graph5 = ["\t4: ss_toldat;"] # grafika switch
                        cs_graph6 = [f"\t5: ss_{ItemID}_{Color}_c;"] # grafika switch
    ...
    lines.append(f"\t\tpurchase:            ss_{ItemID}_{Color}_purchase;")
